...
{
    hs_queuePointerCheck(pQueue);
    hs_arrayAdd(pQueue -> elements, element);
    ++(pQueue -> size);
}
...
{
    hs_queuePointerCheck(pQueue);
    HS_ELEMENT_TYPE element = hs_arrayRemoveAtIndex(pQueue -> elements, 0);
    --(pQueue -> size);
    return element;
}
...
{
    hs_queuePointerCheck(pQueue);
    return hs_arrayGetOfIndex(pQueue -> elements, 0);
}
